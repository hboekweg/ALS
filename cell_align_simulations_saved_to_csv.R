...
false_positive = function(sd, number_of_subsamples, number_of_cells){
  ...
  colnames(Yflat) = paste0("t", 1:1000)
  rownames(Yflat) = "gene"
  colnames(Yslope) = paste0("t", 1:1000)
  rownames(Yslope) = "gene"
  
  X = t(X)
  rownames(X) = paste0("t", 1:1000)
  colnames(X) = NULL
  #I know this line of code below is bad. yslope is assigned to yflat so we can use the subsample function, which calls the oracle Y, and the untruth Yflat. But in this case Yflat is the oracle, so we had to switch them.
  data = list("time_traj" = X, "Y" = Yflat, "flat_Y"=Yslope)

  
  
  #interpolate
  inter_flat = interpolate(X, Yflat, subsample=FALSE) 
  inter_slope = interpolate(X, Yslope, subsample=FALSE) 
  
  slope_area = c()
  flat_area = c()
  for(i in 1:number_of_subsamples){
    mysample = subsample(oracle =data , number_of_cells)
    inter_S = interpolate(mysample$traj, mysample$exp, subsample=TRUE)
    
    flat = find_area(inter_flat$interpolated$traj, inter_flat$interpolated$value, inter_S$interpolated$value, inter_flat$interpolated$traj, inter_S$interpolated$traj, inter_S)
    flat_area[i] = flat
    slope = find_area(inter_slope$interpolated$traj, inter_slope$interpolated$value, inter_S$interpolated$value, inter_slope$interpolated$traj, inter_S$interpolated$traj, inter_S)
    slope_area[i] = slope
  }
  
  areas = list("flat_area"=flat_area, "slope_area"=slope_area)
  return(areas)
}
...
ggplot(inter_S$interpolated, aes(x=traj,y=value)) +  geom_errorbar(aes(ymin=value-error/2, ymax=value+error/2)) + 
  ...
  geom_line(data=inter_slope$interpolated, color='blue')
...
cells_7 = do.call("rbind", list(df_slope.5, df_slope1, df_slope2,df_slope4))
dir.create("june12")
write.csv(cells_7, "june12/cells_7")
...
write.csv(cells_16, "june12/cells_16")
...
write.csv(cells_20, "june12/cells_20")
...
write.csv(cells_30, "june12/cells_30")


#### False Positives ####
sd0 = false_positive(sd=0, number_of_cells=7, number_of_subsamples=1000)
sd25 = false_positive(sd=.25, number_of_cells=7, number_of_subsamples=1000)
sd5 = false_positive(sd=.5, number_of_cells=7, number_of_subsamples=1000)
sd7 = false_positive(sd=.7, number_of_cells=7, number_of_subsamples=1000)
sd1 = false_positive(sd=1, number_of_cells=7, number_of_subsamples=1000)
FP_sd0 = accuracy_perc(sd0$flat_area, sd0$slope_area)
FP_sd25 = accuracy_perc(sd25$flat_area, sd25$slope_area)
FP_sd5 = accuracy_perc(sd5$flat_area, sd5$slope_area)
FP_sd7 = accuracy_perc(sd7$flat_area, sd7$slope_area)
FP_sd1 = accuracy_perc(sd1$flat_area, sd1$slope_area)
FP_cells7 = data.frame("deviation"=c(0, .25, .5,.7,1), "accuracy"=c(FP_sd0, FP_sd25, FP_sd5, FP_sd7 ,FP_sd1), cells=c(7, 7, 7, 7, 7))


sd0 = false_positive(sd=0, number_of_cells=16, number_of_subsamples=1000)
sd25 = false_positive(sd=.25, number_of_cells=16, number_of_subsamples=1000)
sd5 = false_positive(sd=.5, number_of_cells=16, number_of_subsamples=1000)
sd7 = false_positive(sd=.7, number_of_cells=16, number_of_subsamples=1000)
sd1 = false_positive(sd=1, number_of_cells=16, number_of_subsamples=1000)
FP_sd0 = accuracy_perc(sd0$flat_area, sd0$slope_area)
FP_sd25 = accuracy_perc(sd25$flat_area, sd25$slope_area)
FP_sd5 = accuracy_perc(sd5$flat_area, sd5$slope_area)
FP_sd7 = accuracy_perc(sd7$flat_area, sd7$slope_area)
FP_sd1 = accuracy_perc(sd1$flat_area, sd1$slope_area)
FP_cells16 = data.frame("deviation"=c(0, .25, .5,.7,1), "accuracy"=c(FP_sd0, FP_sd25, FP_sd5, FP_sd7 ,FP_sd1), cells=c(16, 16, 16, 16, 16))

sd0 = false_positive(sd=0, number_of_cells=20, number_of_subsamples=1000)
sd25 = false_positive(sd=.25, number_of_cells=20, number_of_subsamples=1000)
sd5 = false_positive(sd=.5, number_of_cells=20, number_of_subsamples=1000)
sd7 = false_positive(sd=.7, number_of_cells=20, number_of_subsamples=1000)
sd1 = false_positive(sd=1, number_of_cells=20, number_of_subsamples=1000)
FP_sd0 = accuracy_perc(sd0$flat_area, sd0$slope_area)
FP_sd25 = accuracy_perc(sd25$flat_area, sd25$slope_area)
FP_sd5 = accuracy_perc(sd5$flat_area, sd5$slope_area)
FP_sd7 = accuracy_perc(sd7$flat_area, sd7$slope_area)
FP_sd1 = accuracy_perc(sd1$flat_area, sd1$slope_area)
FP_cells20 = data.frame("deviation"=c(0, .25, .5,.7,1), "accuracy"=c(FP_sd0, FP_sd25, FP_sd5, FP_sd7 ,FP_sd1), cells=c(20, 20, 20, 20, 20))

sd0 = false_positive(sd=0, number_of_cells=30, number_of_subsamples=1000)
sd25 = false_positive(sd=.25, number_of_cells=30, number_of_subsamples=1000)
sd5 = false_positive(sd=.5, number_of_cells=30, number_of_subsamples=1000)
sd7 = false_positive(sd=.7, number_of_cells=30, number_of_subsamples=1000)
sd1 = false_positive(sd=1, number_of_cells=30, number_of_subsamples=1000)
FP_sd0 = accuracy_perc(sd0$flat_area, sd0$slope_area)
FP_sd25 = accuracy_perc(sd25$flat_area, sd25$slope_area)
FP_sd5 = accuracy_perc(sd5$flat_area, sd5$slope_area)
FP_sd7 = accuracy_perc(sd7$flat_area, sd7$slope_area)
FP_sd1 = accuracy_perc(sd1$flat_area, sd1$slope_area)
FP_cells30 = data.frame("deviation"=c(0, .25, .5,.7,1), "accuracy"=c(FP_sd0, FP_sd25, FP_sd5, FP_sd7 ,FP_sd1), cells=c(30, 30, 30, 30, 30))


FP = do.call("rbind", list(FP_cells7, FP_cells16, FP_cells20, FP_cells30))
write.csv(FP, "june12/FP")
